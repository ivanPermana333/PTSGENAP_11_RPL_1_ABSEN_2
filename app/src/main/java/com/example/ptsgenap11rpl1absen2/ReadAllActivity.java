...
public class ReadAllActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        dataBarang = new ArrayList<>(); //arraylist untuk menyimpan data mahasiswa
    ...
    public void getData(){
        ...
        AndroidNetworking.get("http://192.168.43.92/crud_Android/read_all.php")
                        ...
                        // do anything with response
                        {
                            ...
                            try {
                                for (int i = 0; i < response.length(); i++) {
                                    ...
                                    item.setKode(data.getString("kode"));
                                    item.setJenisbarang(data.getString("jenisbarang"));
                                    item.setNamabarang(data.getString("namabarang"));

                                    dataBarang.add(item);
                                }
                                //men inisialisasi adapter RecyclerView yang sudah kita buat sebelumnya
                                ListBarangAdapter adapter = new ListBarangAdapter(ReadAllActivity.this, dataBarang, ReadAllActivity.this);
